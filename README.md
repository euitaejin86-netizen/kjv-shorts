...
1. Claude Code에 이렇게 말한다:
   ...
   **Claude Code는 주제와 내용을 임의로 정하지 않는다.** 대본을 쓰기 전에 반드시
   먼저 물어본다:

   - **어떤 주제로 할지** — `topics.json`의 목록에서 제안하되 결정은 사용자가 한다.
     목록에 없는 주제도 된다.
   - **어떤 내용이 들어갈지** — 어느 구절을 쓸지, 어떤 각도로 풀지, 무엇을
     강조할지. 제안을 내놓고 승인을 받은 뒤에 쓴다.

   이건 취향 문제가 아니다. 이 채널은 성경 교리를 다루므로 어떤 구절을 어떤
   맥락으로 엮느냐가 곧 무엇을 가르치느냐가 된다. 그 결정은 사용자 몫이다.

   승인을 받은 뒤에야 `corpus/`를 grep해 근거 자료를 참고하고,
   `bible/kjv_ko.json` / `bible/kjv_en.json`에서 구절을 그대로 인용해
   `scripts/`에 대본 JSON을 쓴다.
